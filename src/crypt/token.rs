...
pub struct Token {
	...
	pub sign_b64u: String, // Signature, b64url encoded.
}
...
// region:   --- (private) TokenGen and Validation
fn _generate_token(
	ident: &str,
	duration_sec: f64,
	salt: &str,
...
// endregion:   --- (private) TokenGen and Validation
...
mod tests {
	...
	fn test_token_display_ok() -> Result<()> {
		// -- Fixtures
		let fx_token_str = "ZngtaWRlbnQtMDE.MjAyNC0wNS0xN1QxNTozMDowMFo.some-sign-b64u-encoded";
		let fx_token = Token {
	...
	fn test_validate_web_token_ok() -> Result<()> {
		...
		// -- Check
		_validate_token_sign_and_exp(&fx_token, fx_salt, token_key)?;
	...
	fn test_validate_web_token_err_expired() -> Result<()> {
		...
		let res = _validate_token_sign_and_exp(&fx_token, fx_salt, token_key);
